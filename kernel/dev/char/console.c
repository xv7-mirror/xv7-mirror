...
void panic(char* s)
{
    ...
    cons.locking = 0;
    cprintf("\nxv7 has panicked.\n");
    cprintf("Below is some info that you can submit to "
...
void consolecls()
{
    ...
    for (i = 0; i < 25 * 80; i++)
        crt[i]
            = ' ' | 0x0700; /* Space with normal attribute (white on black) */
...
int consoleioctl(struct inode* ip, int request, void* arg)
{
    switch (request) {
