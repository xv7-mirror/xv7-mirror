 ...
 * with the original xv6 code. It also utilizes spinlocks and sets the baud
 * rate, which is helpful.
 */
...
void uartputc(int c)
{
     ...
     * uartputc() to not waste time calling this function, and instead expose
     * the uart_present flag to functions like cprintf(). This would save the
     * user a bit (in fact, a few nanoseconds) of time.
     */
    if (!uart_present)
        return;
