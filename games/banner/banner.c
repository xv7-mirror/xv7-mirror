...
/** @mainpage Banner Program
  ...
  * something like this anywhere (well, there is @c /usr/games/banner, but
  that's
 ...
 * The array itself is not freed, only the allocations for the individual
 * elements.
 ...
 * @return Boolean true (1) if output was truncated, boolean false (0)
 * otherwise.
 */

static int add_to_banner(
    char** banner, int height, int space, char letter, int maxwidth)
{
    int i = 0;
    int j = 0;
    ...
    for (i = 0; i < height; i++) {
        int new_length
            = strlen(banner[i]) + strlen(working_char[i]) + space + 1;
...
static void fill_banner(char** banner, char* string, int maxwidth)
{
    ...
    for (j = 0; j < strlen(string); j++) {
        truncated = add_to_banner(
            banner, LETTER_HEIGHT, SPACE_WIDTH, string[j], maxwidth);
        if (truncated)
            break;
    }
}
